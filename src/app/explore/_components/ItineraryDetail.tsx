...
const ItineraryDetail = memo(function ItineraryDetail({
  itinerary,
}: {
  itinerary: Itinerary;
}) {
  // Memoize the stops mapping to prevent unnecessary recalculations
  const stopsContent = useMemo(() => {
    return itinerary.stops?.map((stop: Stop, index: number) => (
...
ItineraryDetail.displayName = 'ItineraryDetail';
