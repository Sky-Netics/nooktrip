...
const ItineraryCard = memo(function ItineraryCard({
  ...
  return (
    <div className="relative grid px-6 pt-4 pb-5 rounded-md bg-card shadow-sm border border-muted">
      ...
      <span className="max-sm:mt-4 sm:absolute right-6 top-4 text-primary-foreground font-extrabold">
        {`${itinerary.total_cost} CAD`}
      </span>
      ...
      >
        <Button size="sm">
          View Details
          <ChevronRight className="w-4 h-4 hidden sm:block ml-1" />
        </Button>
      </Link>
    </div>
  );
